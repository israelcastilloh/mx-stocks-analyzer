...
from utils import tickers_from_market, download_prices, dividends
...
def update_returns_figure(input_value, window_value):
    wb_prices = historicos
    price_data = wb_prices[input_value].tail(window_value)
    ...
    market_prices = wb_prices['^MXX'].tail(window_value)
    ...
    fig2 = make_subplots(rows=1, cols=2, horizontal_spacing=0.05,
                         subplot_titles=['Cumulative Return', 'Daily Return Histogram'],
                         column_widths=(5, 3))
...
def update_stat_table(input_value, window_value):
    wb_prices = historicos
    price_data = wb_prices[input_value]
    price_data.reset_index(level=0, inplace=True)
